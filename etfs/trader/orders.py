...
def put_in_stop_loss_orders_all(access_token=None, username=None, password=None, portfolio=None, column="Close", alpha=.5, sigmas=2):
    ...
    for ticker in portfolio.tickers:
        ...
        if (portfolio.positions_df.Quantity[ticker] > 0) & (portfolio.positions_df.Quantity[ticker] == float(stock_data['quantity'])):
            portfolio.securities[ticker].latest_price = float(r.get_latest_price([ticker])[0])
            portfolio.securities[ticker].stop_loss_price = round(get_stop_loss_price(security=portfolio.securities[ticker], column=column, alpha=alpha, sigmas=sigmas),2)
            ...
            print(ticker, 
                  ...
                  portfolio.securities[ticker].stop_loss_price - portfolio.securities[ticker].last_price_ewm,
                  100*(portfolio.securities[ticker].stop_loss_price - portfolio.securities[ticker].latest_price)/portfolio.securities[ticker].latest_price
                 )
