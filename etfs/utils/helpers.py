...
def todays_date():
    return '{0}{1:02}{2:02}'.format(dt.datetime.now().year, dt.datetime.now().month, dt.datetime.now().day)
...
def standard_date_format(input_date):
    ...
    if type(input_date) != str:
        input_date = '{0}-{1:02}-{2:02}'.format(input_date.year, input_date.month, input_date.day)
