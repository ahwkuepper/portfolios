...
def plot_security_performance(portfolio=None, ticker=None):
    ...
    """
    _df = portfolio.transactions.loc[(portfolio.transactions.Ticker == portfolio.securities_archive[ticker].ticker) & \
                                (portfolio.transactions.Transaction.isin(('buy', 'sell'))), ['Date', 'Transaction', 'Quantity', 'TradeValue']].copy()
    ...
    _df = portfolio.data[['Total', ticker]].join(_df[['Quantity', 'TradeValue', 'AvgPrice']], how='left', rsuffix='').fillna(method='ffill')
    ...
    if min(_df.AvgValue.min(), _df.AvgPrice.min()) < 0 or abs(min(_df.AvgValue.min(), _df.AvgPrice.min()))/max(_df.AvgValue.max(), _df.AvgPrice.max()) < .1:
    ...
    if min(_df[ticker].min(), _df.TradeValue.min()) < 0 or abs(min(_df[ticker].min(), _df.TradeValue.min()))/max(_df[ticker].max(), _df.TradeValue.max()) < .1:
        axes[1].axhline(y=0, linestyle=':')
    axes[1].legend()

    # return
    axes[0].plot(_df.index, _df[ticker].values-_df.TradeValue.values, label='{} return'.format(ticker))
